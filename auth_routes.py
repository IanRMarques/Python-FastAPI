...
async def create_user(email: str,senha: str, nome:str, session = Depends(pegar_sessao)):
    ...
    if usuario:
        ...
        senha_criptografada = bcrypt_context.hash(senha) #criptografando a senha fornecida usando o objeto bcrypt_context configurado para usar o algoritmo bcrypt
        novo_usuario = Usuario(nome, email, senha_criptografada) #criando uma nova instância da classe Usuario com o nome, email e senha criptografada fornecidos
